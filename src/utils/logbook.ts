...
export async function getUserProposalIds(logbook: LogbookController, userId: string): Promise<ProposalModel> {
  
  let options = {};

  try {
    const user = await User.findById(userId);
    const userIdentity = await userRepository.findOne({
      where: { userId },
    });
